...
export class WikiRetriever {
    private static knowledge: any = null;

    private static loadKnowledge() {
        if (this.knowledge && Object.keys(this.knowledge).length > 0) return;
        try {
    ...
    static search(query: string): string[] {
        ...
        for (const [category, topics] of Object.entries(this.knowledge)) {
            for (const [topic, content] of Object.entries(topics as Record<string, string>)) {
