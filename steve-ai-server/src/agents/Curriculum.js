...
export class Curriculum {
    ...
    async proposeNextTask(obs) {
        const context = this.memory.search(obs.lastTask || "survival", 3).join("\n");
        // Inject RAG: tool tier hint và Wiki Insights
        const toolHint = this.buildToolHint(obs.inventory);
        const wikiInsight = KnowledgeBase.getFullContext(obs.biome || obs.dimension || "survival");
...
${wikiInsight || "No specific wiki data for current context."}
