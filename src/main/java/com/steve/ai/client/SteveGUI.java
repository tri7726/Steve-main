 ...
 */
@SuppressWarnings("null") // Minecraft client API (font, GuiGraphics, EntityRenderer) guaranteed non-null at runtime
public class SteveGUI {
    ...
    public static void onRenderOverlay(RenderGuiOverlayEvent.Post event) {
        ...
        for (ChatMessage msg : messages) {
            ...
            // Count wrapped lines so the bubble height reflects actual content size
            int lineCount = wrappedText.isEmpty() ? 1 : wrappedText.split("\n", -1).length;
            int bubbleHeight = (MESSAGE_HEIGHT * lineCount) + 10; // bubble padding
    ...
    public static void handleMouseClick(double mouseX, double mouseY, int button) {
        ...
        int screenWidth = mc.getWindow().getGuiScaledWidth();
        int screenHeight = mc.getWindow().getGuiScaledHeight();
        // Calculate panel's left edge (accounting for slide animation)
        int panelX = (int) (screenWidth - PANEL_WIDTH + slideOffset);

        // Only process clicks within the panel bounds
