...
public class StructureTemplateLoader {
    ...
    public static LoadedTemplate loadFromNBT(ServerLevel level, String structureName) {
        ...
        try {
            ResourceLocation resourceLocation = ResourceLocation.parse(SteveMod.MODID + ":" + structureName);
     ...
     *
     * @deprecated Not fully implemented. All structure loading should go through
     *             {@link #load(ResourceLocation)} which uses NBT files.
     *             Size info is available via {@code template.getSize()} if needed in future.
     */
    @Deprecated
    private static LoadedTemplate loadFromMinecraftTemplate(StructureTemplate template, String name) {
        ...
        // template.getSize() gives (width, height, depth) as a Vec3i, but
        // iterating block info requires palette access that depends on Forge internals.
        // Use NBT files (.nbt) and loadFromFile() instead.
        SteveMod.LOGGER.warn("Direct template loading not fully implemented for '{}', use NBT files directly", name);
        return null;
    }
    ...
    private static LoadedTemplate parseNBTStructure(CompoundTag nbt, String name) {
        ...
        for (int i = 0; i < paletteList.size(); i++) {
            ...
            try {
                ...
                Block block = net.minecraftforge.registries.ForgeRegistries.BLOCKS.getValue(blockLocation);
